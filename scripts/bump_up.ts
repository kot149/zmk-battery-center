...
async function updatePackageJson(versionNumber: string): Promise<void> {
  ...
  content = content.replace(/"version"\s*:\s*"[\d.]+"/, `"version": "${versionNumber}"`);
...
async function updateCargoToml(versionNumber: string): Promise<void> {
  ...
  content = content.replace(/^version\s*=\s*"[\d.]+"/m, `version = "${versionNumber}"`);
...
async function updateCargoLock(versionNumber: string): Promise<void> {
  execSync("cargo metadata --manifest-path src-tauri/Cargo.toml --format-version 1 --no-deps", {
  ...
  while (Date.now() <= deadline) {
    ...
    await new Promise((resolve) => setTimeout(resolve, CARGO_LOCK_POLL_INTERVAL_MS));
  }

  throw new Error(`Timed out waiting for ${CARGO_LOCK_PATH} to contain version ${versionNumber}.`);
}
...
function printNextSteps(version: string): void {
  ...
  console.log("\x1b[32mTo push changes to remote, run the following commands:\x1b[0m");
...
async function main(): Promise<void> {
  ...
  if (!/^v\d+\.\d+\.\d+$/.test(version)) {
    console.error('Error: Version must be in the format "vx.x.x". Example: v1.2.3');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!skipGit) {
      ...
      console.log(`\nVersion update to ${version} completed!`);
      console.log("Note: Git commit and tag were not created due to --no-git flag.");
    }
  } catch (error) {
