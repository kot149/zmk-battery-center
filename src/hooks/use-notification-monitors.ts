...
export function useNotificationMonitors({
  ...
  useEffect(() => {
    ...
    const syncNotificationMonitors = async () => {
      ...
      const desired = isNotificationMonitorMode ? new Set(desiredIds) : new Set<string>();
    ...
    syncChainRef.current = syncChainRef.current
      ...
      .catch((e) =>
        logger.warn(`Failed to synchronize battery notification monitors: ${String(e)}`),
      );

    return () => {
      // Bump the generation so an in-flight run stops mutating state.
      // (A newer effect run bumps it anyway; this covers unmount.)
      syncGenerationRef.current++; // oxlint-disable-line react-hooks/exhaustive-deps -- plain counter ref, not a DOM node
    };
  }, [
    registeredDeviceIdsKey,
    isNotificationMonitorMode,
