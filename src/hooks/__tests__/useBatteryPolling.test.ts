...
const connectedBatteryInfos: BatteryInfo[] = [{ battery_level: 50, user_description: "Central" }];
...
type RenderOverrides = Partial<
  Omit<PollingOptions, "registeredDevicesRef" | "commitRegisteredDevices">
> & {
...
function renderPolling(overrides: RenderOverrides = {}) {
  ...
  const commitRegisteredDevices = vi.fn(
...
describe("useBatteryPolling", () => {
  ...
  it("records readings and forwards edge transitions on success", async () => {
    const device = createDevice();
    const fetchedInfos: BatteryInfo[] = [{ battery_level: 18, user_description: "Central" }];
    ...
    expect(recordBatteryReadings).toHaveBeenCalledWith(device, [
      expect.objectContaining(fetchedInfos[0]),
    ]);
    expect(notifyBatteryEdgeTransitions).toHaveBeenCalledOnce();
    expect(notifyBatteryEdgeTransitions).toHaveBeenCalledWith(
  ...
  it("success commits merged battery infos and clears isDisconnected", async () => {
    const device = createDevice({ isDisconnected: true });
    const fetchedInfos: BatteryInfo[] = [{ battery_level: 75, user_description: "Central" }];
    vi.mocked(getBatteryInfo).mockResolvedValue(fetchedInfos);
    const view = renderPolling({ device, isPollingMode: false });
