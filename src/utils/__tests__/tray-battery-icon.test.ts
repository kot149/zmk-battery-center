...
describe("trayBatteryPayloadFromPrimaryDevice", () => {
  ...
  it("keeps a disconnected device with no infos enabled with a default label", () => {
    const payload = trayBatteryPayloadFromPrimaryDevice([device({ isDisconnected: true })]);
  ...
  it("emits uppercase single-character glyphs for every label source", () => {
    const cases = [
      ...
      {
        registeredDevice: device({ batteryInfos: [info(90, "central")] }),
        expectedLabels: ["C", null],
      },
      {
        registeredDevice: device({ batteryInfos: [info(90, "peripheral")] }),
        expectedLabels: ["P", null],
      },
      {
        registeredDevice: device({ batteryInfos: [info(90, "Left half")] }),
        expectedLabels: ["L", null],
      },
