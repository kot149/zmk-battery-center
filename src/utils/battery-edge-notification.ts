...
export function notifyBatteryEdgeTransitions({
  ...
  const notify = (
    ...
    for (let i = 0; i < curr.length && i < prev.length; i++) {
      ...
      const hasCustomPartLabel =
        partDisplayName !== defaultBatteryPartDisplayName(part.user_description);
      const suffix = newBatteryInfos.length >= 2 || hasCustomPartLabel ? " " + partDisplayName : "";
  ...
  const mapLowBatteryNotificationState = (batteryInfos: BatteryInfo[]) =>
    mapIsLowBattery(batteryInfos, lowBatteryThreshold).map(
      (isLow, index) => isLow && (!ignoreZeroPercent || batteryInfos[index].battery_level !== 0),
    );
